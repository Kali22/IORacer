...
void ObjectManager::LoadObjects(b2World *world, std::string map_name, float scale) {
    std::ifstream description("../resource/maps/" + map_name + "/description");
    ...
    while (description >> type >> x >> y >> angle) {
        switch (type) {
            case 'b':
                objects_.push_back(std::make_shared<Box>(world, x, y, angle, scale));
                break;
            case 't':
                objects_.push_back(std::make_shared<TruckTire>(world, x, y, angle, scale));
                break;
            default:
